...
dist = zeros(size(test,1),k+2); %that's the distance from a centroid, and labels

for i = 1:size(test,1)
       for j = 1:k     
           dist(i,j) = norm(test(i,:) - centroids(j,:));  

       end   
     [Distance, CN] = min(dist(i,1:k));               % 1:K are Distance from Cluster Centers 1:K 
      dist(i,k+1) = CN;                               % K+1 is Cluster Label
      dist(i,k+2) = Distance;                         % K+2 is Minimum Distance
end    
...
function [index, vec_distance] = assign_vector_to_centroid(data,centroids) %input data & centroids, out index & dis
